...
let zhTranslations = {
    ...
    'type': '类型',
    'vendor': '供应商',
    'cookie_scanning': 'Cookie扫描',
    'steps_to_scan_cookies': '应用团队成员可采取如下步骤扫描Cookie。开始之前，在您的浏览器侧，请确保应用域名指向网关的主节点，如果没有，可修改本地hosts文件(C:\\Windows\\System32\\drivers\\etc\\hosts)。',
    'open_website': '步骤1：用户侧，用浏览器打开目标网站，如果没有出现Cookie偏好设置窗口，请点击窗口左下角的Cookie图标',
    'enable_unclassified_and_navigate': '步骤2：用户侧，在Cookie偏好设置窗口，启用未分类Cookie并确认，然后浏览可能设置Cookie的页面',
    'refresh_and_check_cookies': '步骤3：网关侧，点击上方刷新按钮，查看识别出的Cookie清单，检查或修改每一Cookie类型，直至未分类Cookie数量清零',
    'navigate_website': '步骤4：用户侧，浏览目标网站，检查Cookie设置是否生效',
    // CookieRef
